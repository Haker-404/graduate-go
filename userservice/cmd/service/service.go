...
import (
	...
	grpc "awesomeProject/userservice/pkg/grpc"
	pb "awesomeProject/userservice/pkg/grpc/pb"
	http "awesomeProject/userservice/pkg/http"
	"awesomeProject/userservice/pkg/service"
	"flag"
	"fmt"
	"net"
	http1 "net/http"
...
var fs = flag.NewFlagSet("userservice", flag.ExitOnError)
...
func Run() {
	...
	if *zipkinURL != "" {
		...
		//recorder := zipkingoopentracing.NewRecorder(collector, false, "localhost:80", "userservice")
		...
		endpoint, err := zipkin.NewEndpoint("UserService", "localhost:8082")
		if err != nil {
