...
func makeGetUserListHandler(m *mux.Router, endpoints endpoint.Endpoints, options []http.ServerOption) {
	m.Methods("POST").Path("/get-user-list").Handler(handlers.CORS(handlers.AllowedMethods([]string{"POST"}), handlers.AllowedOrigins([]string{"*"}))(http.NewServer(endpoints.GetUserListEndpoint, decodeGetUserListRequest, encodeGetUserListResponse, options...)))
}

// decodeGetUserListRequest is a transport/http.DecodeRequestFunc that decodes a
// JSON-encoded request from the HTTP request body.
func decodeGetUserListRequest(_ context.Context, r *http1.Request) (interface{}, error) {
	req := endpoint.GetUserListRequest{}
	err := json.NewDecoder(r.Body).Decode(&req)
	return req, err
}

// encodeGetUserListResponse is a transport/http.EncodeResponseFunc that encodes
// the response as JSON to the response writer
func encodeGetUserListResponse(ctx context.Context, w http1.ResponseWriter, response interface{}) (err error) {
